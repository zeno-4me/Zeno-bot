...
def init_db():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS guild_config (guild_id INTEGER PRIMARY KEY, image_points REAL DEFAULT 1.0, gif_points REAL DEFAULT 2.0, video_points REAL DEFAULT 1.0, video_minute_points REAL DEFAULT 4.0, allowed_channels TEXT DEFAULT '', allowed_categories TEXT DEFAULT '')")
    cursor.execute("CREATE TABLE IF NOT EXISTS user_points (guild_id INTEGER, user_id INTEGER, points REAL DEFAULT 0.0, images_count INTEGER DEFAULT 0, gifs_count INTEGER DEFAULT 0, videos_count INTEGER DEFAULT 0, video_seconds INTEGER DEFAULT 0, PRIMARY KEY (guild_id, user_id))")
    cursor.execute("CREATE TABLE IF NOT EXISTS tracked_messages (message_id INTEGER PRIMARY KEY, guild_id INTEGER, user_id INTEGER, points_awarded REAL, media_type TEXT, duration_sec INTEGER)")
    conn.commit()
    conn.close()
...
async def get_video_duration(url: str) -> float:
    ...
    try:
        ...
        print(f"Error video duration: {e}")
    finally:
...
async def on_message(message: discord.Message):
    ...
    ch_ok = not config["allowed_channels"] or message.channel.id in config["allowed_channels"]
    cat_ok = not config["allowed_categories"] or (message.channel.category_id and message.channel.category_id in config["allowed_categories"])
    ...
    if detected_media:
        ...
        tot_pts = sum(m[1] for m in detected_media)
        tot_imgs = sum(1 for m in detected_media if m[0] == 'image')
        tot_gifs = sum(1 for m in detected_media if m[0] == 'gif')
        tot_vids = sum(1 for m in detected_media if m[0] == 'video')
        tot_secs = sum(m[2] for m in detected_media if m[0] == 'video')

        cursor.execute("INSERT INTO user_points (guild_id, user_id, points, images_count, gifs_count, videos_count, video_seconds) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET points = points + ?, images_count = images_count + ?, gifs_count = gifs_count + ?, videos_count = videos_count + ?, video_seconds = video_seconds + ?", (message.guild.id, message.author.id, tot_pts, tot_imgs, tot_gifs, tot_vids, tot_secs, tot_pts, tot_imgs, tot_gifs, tot_vids, tot_secs))

        for m_type, pts, dur in detected_media:
            cursor.execute("INSERT INTO tracked_messages (message_id, guild_id, user_id, points_awarded, media_type, duration_sec) VALUES (?, ?, ?, ?, ?, ?)", (message.id, message.guild.id, message.author.id, pts, m_type, dur))
...
async def on_message_delete(message: discord.Message):
    ...
    if records:
        ...
        cursor.execute("UPDATE user_points SET points = max(0, points - ?), images_count = max(0, images_count - ?), gifs_count = max(0, gifs_count - ?), videos_count = max(0, videos_count - ?), video_seconds = max(0, video_seconds - ?) WHERE guild_id = ? AND user_id = ?", (del_pts, del_imgs, del_gifs, del_vids, del_secs, message.guild.id, message.author.id))
        cursor.execute("DELETE FROM tracked_messages WHERE message_id = ?", (message.id,))
        conn.commit()

    conn.close()

@bot.tree.command(name="setup_points", description="تعديل آلية احتساب النقاط")
...
async def setup_points(interaction: discord.Interaction, image_points: float, gif_points: float, video_points: float, video_minute_points: float):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO guild_config (guild_id, image_points, gif_points, video_points, video_minute_points) VALUES (?, ?, ?, ?, ?) ON CONFLICT(guild_id) DO UPDATE SET image_points = ?, gif_points = ?, video_points = ?, video_minute_points = ?", (interaction.guild_id, image_points, gif_points, video_points, video_minute_points, image_points, gif_points, video_points, video_minute_points))
    conn.commit()
    conn.close()

    embed = discord.Embed(title="⚙️ تم تحديث الإعدادات", color=discord.Color.green())
    embed.add_field(name="الصورة", value=f"`{image_points}`", inline=True)
    embed.add_field(name="GIF", value=f"`{gif_points}`", inline=True)
    embed.add_field(name="الفيديو", value=f"`{video_points}`", inline=True)
    embed.add_field(name="الدقيقة", value=f"`{video_minute_points}`", inline=True)
...
async def points(interaction: discord.Interaction, user: discord.Member = None):
    ...
    embed.add_field(name="🖼️ الصور", value=f"`{imgs}`", inline=False)
    embed.add_field(name="🎥 الفيديوهات", value=f"`{vids}` ({total_minutes} دقيقة)", inline=False)
    embed.add_field(name="GIF", value=f"`{gifs}`", inline=False)
...
async def leaderboard(interaction: discord.Interaction):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, points FROM user_points WHERE guild_id = ? ORDER BY points DESC LIMIT 10", (interaction.guild_id,))
    rows = cursor.fetchall()
    conn.close()

    if not rows:
        await interaction.response.send_message("لا يوجد بيانات مسجلة.")
        return

    embed = discord.Embed(title="🏆 قائمة الصدارة", color=discord.Color.gold())
    desc = ""
    for idx, (user_id, pts) in enumerate(rows, 1):
        member = interaction.guild.get_member(user_id)
        name = member.mention if member else f"عضو مغادر ({user_id})"
        desc += f"**#{idx}** | {name} - **{round(pts, 2)}** نقطة\n"
...
async def on_message(message: discord.Message):
    ...
    # Save Batch to Database (Fix for Bulk Uploads)
    if detected_media:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        ...
        ''', (message.guild.id, message.author.id, tot_pts, tot_imgs, tot_gifs, tot_vids, tot_secs,
              tot_pts, tot_imgs, tot_gifs, tot_vids, tot_secs))
